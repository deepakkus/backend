...
const create = async (req, res) => {
  ...
  const menuCategory = await new MenuCategory(menuCategoryData);
  return menuCategory
    .save()
    .then((data) => {
  ...
  return MenuCategory.findOneAndUpdate(
    ...
    async (err, data) => {
      if (err) {
        ...
        return res.status(200).json({
          status: true,
          message: "MenuCategory update successful",
          data: data,
        });
      } else {
        return res.status(500).json({
          status: false,
          message: "MenuCategory not match",
          data: null,
        });
      }
    }
...
const viewAll = async (req, res) => {
  return MenuCategory.aggregate([
    ...
    .then((data) => {
      return res.status(200).json({
        status: true,
        message: "Get All MenuCategory  Successfully",
        data: data,
      });
    })
    .catch((error) => {
...
const DeleteParmanet = async (req, res) => {
  return MenuCategory.remove({ _id: { $in: [mongoose.Types.ObjectId(req.params.id)] } })
    .then((data) => {
      return res.status(200).json({
        success: true,
        message: 'MenuCategory delete successfully',
        data: data
      });
    })
    .catch((error) => {
  ...
  return MenuCategory.findOneAndUpdate(
    ...
    async (err, data) => {
      if (err) {
        ...
        return res.status(200).json({
          status: true,
          message: "MenuCategory Delete successful",
          data: data,
        });
      } else {
        return res.status(500).json({
